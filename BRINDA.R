#
# BRINDA package information ---------------------------------------------------
#

# Title: Biomarker Reflecting Inflammation and Nutrition Determinant of Anemia Adjustment Method
# Imports: plyr, dplyr, tidyr, data.table, stringr, rlang, berryFunctions, Hmisc
# License: Common Creative License 4.0
# Version: 1.0.1
# Bug Reports: Hanqi.luo@emory.edu
# Encoding: UTF-8
# Author: Hanqi Luo, Yaw Addo, Afrin Jahan
# Maintainer: Hanqi Luo (Hanqi.luo@emory.edu)
# Repository: CRAN

# update: 07/27/2021
# change subset into dplyr::select
...
packages_list <- c("plyr", "dplyr", "tidyr", "data.table", "stringr", 
                  "rlang", "berryFunctions", "Hmisc")
...
if(sum(!sapply(packages_list, require, character.only = TRUE))){
    stop(paste0(
        "----------------------------------------------
        Required packages cannot be installed 
        Please check whether packages:", 
...
output_dataset <- function(output_format_quo, ori_dataset, brinda_dataset,
    ...
    mn_biomarker_full_name_list <- names(brinda_dataset %>%
                                             dplyr::select(-mn_inflammation_biomarker_variable_name_list))
    ...
    if(output_format_quo == "FULL"){
        ...
        output_dataset <- 
            cbind(
                ...
                brinda_dataset %>%
                    dplyr::select(mn_biomarker_full_name_list))
        } else{
